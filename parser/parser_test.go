...
func TestLine(t *testing.T) {
	cases := []struct {
		...
		},
		{
			"a_*",
			[]node.Node{
				&node.Line{"TextBlock", []node.Inline{node.Text("a_*")}},
			},
		},
	...
	cases := []struct {
		...
		},
		{
			...
			[]node.Node{
				&node.Hat{
					[][]byte{[]byte("a"), []byte("b")},
					&node.Line{"TextBlock", []node.Inline{node.Text("c")}},
				},
			},
		},
		{
...
func TestBlockEscape(t *testing.T) {
	cases := []struct {
		...
		},
		{
			"\\``",
			[]node.Node{
				&node.Line{"TextBlock", []node.Inline{&node.Escaped{"Code", nil}}},
			},
		},
		{
			"\\\\``",
		...
		// verbatim elements
		{
			`\.image`,
			[]node.Node{
				&node.Line{"TextBlock", []node.Inline{node.Text(".image")}},
			},
		},
		{
			`.image\.image`,
